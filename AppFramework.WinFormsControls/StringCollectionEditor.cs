...
namespace AppFramework.WinFormsControls
{
    ...
    {
        public StringCollectionEditor(System.Type type)
            : base(type)
        {
        }
        ...
        private class StringCollectionForm : CollectionEditor.CollectionForm
        {
            ...
            private void OKButton_click(object sender, EventArgs e)
            {
                ...
                string[] textArray = this.textEntry.Text.Split(separator);
                object[] items = base.Items;
                int length = textArray.Length;
                for (int i = 0; i < length; i++)
                {
                    textArray[i] = textArray[i].Trim(trimChars);
                }
                bool flag = true;
                if (length == items.Length)
                {
                    ...
                    while (index < length)
                    {
                        if (!textArray[index].Equals((string)items[index]))
                        {
                            break;
                        }
                        index++;
                ...
                {
                    if ((textArray.Length > 0) && (textArray[textArray.Length - 1].Length == 0))
                    {
                        length--;
                    }
