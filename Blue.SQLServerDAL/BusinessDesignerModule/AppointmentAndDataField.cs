...
namespace Blue.SQLServerDAL.BusinessDesignerModule
{
    ...
    public class AppointmentAndDataField : CorrelatedTableDataAcess, IAppointmentAndDataField
    {
		...
		public decimal Insert(AppointmentAndDataFieldInfo appointmentAndDataFieldInfo)
		{
			...
			sb.Append("FstTimeValue, ScdTimeValue, NextRelation, Sorting)");
			...
			try
            {
				using (DbCommand dbCommand = db.GetSqlStringCommand(sb.ToString()))
				{
					...
					db.AddInParameter(dbCommand, "Sorting", DbType.Int32, appointmentAndDataFieldInfo.Sorting);
					//执行插入操作
