...
namespace Blue.WindowsFormsClient.MyBusinessModule
{
    ...
    public class WorkflowTable
    {
        ...
        /// 通字段值
        /// </summary>
        private IDictionary<decimal, CommonDataFieldValue> commonDataFieldValues = null;
        ...
        public WorkflowTable(decimal instanceId, bool formReadOnly, bool editedState, CustomFormInfo formInfo, ICustomDataFieldContract dataFieldContract, IAssociatedDataFieldContract asDataFieldContract,
            ...
            ICombinedTableContract cmbTableContract, IBusinessInstanceContract instanceContract, Control control)
        {
            ...
            commonDataFieldValues = new Dictionary<decimal, CommonDataFieldValue>();
        }
        ...
        {
            /* 增加系统字段 */
            IList<ExtendedCustomDataFieldInfo> systemExtendedCustomDataFieldInfos = RoleHelper.GetSystemDataFieldInfo(syste);
        ...
        private void LoadDataOnControl(Control control, ExtendedCustomDataFieldInfo extendedCustomDataFieldInfo, DataRow dataRow)
        {
            ...
            DataFieldProperty dataFieldProperty = (DataFieldProperty)extendedCustomDataFieldInfo.DataFieldProperty;
            bool triggerDataFiled = AuthorityHelper.CheckAuthority(extendedCustomDataFieldInfo.DataFieldSetting, (byte)DataFieldSetting.TriggerDataFiled);
            if (triggerDataFiled)
            {
                if (commonDataFieldValues.ContainsKey(extendedCustomDataFieldInfo.DataFieldId))
                {
                    CommonDataFieldValue commonDataFieldValue = commonDataFieldValues[extendedCustomDataFieldInfo.DataFieldId];
                    commonDataFieldValue.DataFieldProperty = dataFieldProperty;
                    commonDataFieldValue.DataFieldType = extendedCustomDataFieldInfo.DataFieldType;
                    commonDataFieldValue.OriginalDataFieldValue = dataRow[physicalName];
                }
                else
                {
                    CommonDataFieldValue commonDataFieldValue = new CommonDataFieldValue()
                    {
                        DataFieldId = extendedCustomDataFieldInfo.DataFieldId,
                        DataFieldProperty = dataFieldProperty,
                        DataFieldType = extendedCustomDataFieldInfo.DataFieldType,
                        OriginalDataFieldValue = dataRow[physicalName]
                    };
                    commonDataFieldValues.Add(extendedCustomDataFieldInfo.DataFieldId, commonDataFieldValue);
                }
            }
            switch (dataFieldProperty)
            {
            ...
            foreach (Control control in groupControl.Controls)
            {
                ...
                switch (physicalDataFieldType)
                {
                    ...
                    case PhysicalDataFieldType.DepartmentDropdownListEnum:
                        dataFieldDataType = DbType.String;
                        dataFieldValue = ((ComboBoxEdit)control).Text.Trim();
                        if (extendedCustomDataFieldInfo.AutoComplete)
                        {
                            SetAdditionalRecord(recordEntity, AdditionalRecordType.DnamicalDep, extendedCustomDataFieldInfo.DataFieldId, dataFieldProperty, extendedCustomDataFieldInfo.DataFieldType, dataFieldValue);
                        }
                        break;
                ...
                if (triggerDataFiled)
                {
                    SetAdditionalRecord(recordEntity, AdditionalRecordType.Trigger, extendedCustomDataFieldInfo.DataFieldId, dataFieldProperty, extendedCustomDataFieldInfo.DataFieldType, dataFieldValue);
                }
        ...
        /// <param name="additionalRecordType"></param>
        /// <param name="dataFieldId"></param>
        /// <param name="dataFieldProperty"></param>
        /// <param name="dataFieldType"></param>
        /// <param name="dataFieldValue"></param>
        private void SetAdditionalRecord(RecordEntity recordEntity, AdditionalRecordType additionalRecordType, decimal dataFieldId, 
            DataFieldProperty dataFieldProperty, byte dataFieldType, object dataFieldValue)
        {            
            IList<CommonDataFieldValue> dataFieldValues = null;
            if (recordEntity.AdditionalData.ContainsKey(additionalRecordType))
            {
                dataFieldValues = recordEntity.AdditionalData[additionalRecordType];
            }
            else
            {
                dataFieldValues = new List<CommonDataFieldValue>();
                recordEntity.AdditionalData.Add(AdditionalRecordType.DnamicalDep, dataFieldValues);
            }
            CommonDataFieldValue commonDataFieldValue = null;
            if (commonDataFieldValues.ContainsKey(dataFieldId))
            {
                commonDataFieldValue = commonDataFieldValues[dataFieldId];
                commonDataFieldValue.UpdatedDataFieldValue = dataFieldValue;
            }
            else
            {
                commonDataFieldValue = new CommonDataFieldValue()
                {
                    DataFieldId = dataFieldId,
                    DataFieldProperty = dataFieldProperty,
                    DataFieldType = dataFieldType,
                    UpdatedDataFieldValue = dataFieldValue
                };
            }
            dataFieldValues.Add(commonDataFieldValue);
        }
    }
}
