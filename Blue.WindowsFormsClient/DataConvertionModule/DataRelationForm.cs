...
{
    public partial class DataRelationForm : TreeLayerForm
    {
        #region 契约接口
        ...
        private readonly DataRelationModule dataRelationModule;
        ...
        public DataRelationForm()
        {
            ...
            dataRelationModule = new DataRelationModule() { CustomDatabaseContract = customDatabaseContract,DataRelationContract = dataRelationContract};
            userControls.Add(dataRelationModule);

            /* 初始化属性 */
            allowedToFirstLevelNode = false; /* 不允许编辑根节点 */
            MaxLevel = 3;  /*  允许最大层次 */
            Tip = "创建数据交换名称成功后请进行字段设置。";
            NullValuePrompt = "请输入数据交换名称查询";
        ...
        /// <param name="e"></param>
        private void DataConvertionModule_Load(object sender, EventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnBeforeSelectOfTreeView(object sender, TreeViewCancelEventArgs e)
        {
            ...
            if (CurrentQueriedState)
            {
                ...
                if (commonNode.NodeId > 0)
                {
                    ...
                    dataRelationModule.Visible = true;
                    TreeNodeShow = dataRelationModule;
                }
                else
                {
                    groupModule.Visible = true;
                    dataRelationModule.Visible = false;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnBeoreQueryClick(object sender, EventArgs e)
        {
            CommonNodeContract = dataRelationContract;
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnBeforeTreeNodeExpand(object sender, TreeViewCancelEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnBeoreCreatedClick(object sender, TreeNodeItemClickEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataRelationForm_OnExchangeClick(object sender, TreeNodeItemClickEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnDeleteClick(object sender, TreeNodeItemClickEventArgs e)
        {
            try
            {
                CommonNode commonNode = e.TreeNode.Tag as CommonNode;
                ...
                switch (combinedNodeType)
                {
                    case CombinedNodeType.ParentCategory:
                        ...
                        if (count > 0)
                        {
                            ...
                            MessageBox.Show(string.Format("{0}节点下有{1}个数据交换名小类，请删除这些数据交换名小类。", commonNode.NodeName, count), "警告信息", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                    ...
                    case CombinedNodeType.ChildCategory:
                        /* 2. 不允许删除有组合表属于该分组的节点 */
                        count = dataRelationContract.GetTotalCountOfChildNode(commonNode.NodeId);
                        if (count > 0)
                        {
                            Cursor = Cursors.Default;
                            MessageBox.Show(string.Format("共有{0}个数据交换名称属于该节点[{1}]，请先删除这些数据交换名。", count, commonNode.NodeName), "警告信息", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                        customGroupContract.Delete(commonNode.NodeId);
                        break;
                    ...
                    case CombinedNodeType.Leaf:
                        dataRelationContract.Delete(commonNode.NodeId);
                        break;
                }
                DeleteNode();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnConfirmClick(object sender, TreeNodeEditEventArgs e)
        {
            bool result = false;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnCancelClick(object sender, TreeNodeEditEventArgs e)
        {
            if (!CurrentQueriedState)
            {
                Cursor = Cursors.WaitCursor;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DataConvertionModule_OnSettingClick(object sender, TreeNodeItemClickEventArgs e)
        {
            ...
            if (nodeType == CombinedNodeType.Leaf)
            {
                ...
                frmTableRelation.DataSourceTypeValue = DataSourceType.Local;
        ...
        private void DataRelationForm_OnCustomItemClick(object sender, TreeNodeItemClickEventArgs e)
        {
            ...
            if (nodeType == CombinedNodeType.Leaf)
            {
                ...
                frmDatatCopy.DataSourceTypeValue = DataSourceType.Local;
            ...
            switch (combinedNodeType)
            {
                ...
                case CombinedNodeType.ParentCategory:
                    ...
                    CustomGroupInfo customGroupInfo = new CustomGroupInfo()
                    {
                        ...
                        GroupType = (byte)GroupType.DataRelation,
                ...
                case CombinedNodeType.ChildCategory:
                    result = dataRelationModule.ValidateModelInfo(out verifyResult);
                    if (result)
                    {
                        DataRelationInfo dataRelationInfo = dataRelationModule.GetModelInfo();
                        dataRelationInfo.GroupId = commonNode.NodeId;
                        if (dataRelationContract.IsExistIdenticalName(commonNode.NodeId, dataRelationInfo.RelationName))
                        {
                            verifyResult = string.Format("同一分类下的数据交换名称不允许重复, 数据交换名称[{0}]已存在。", dataRelationInfo.RelationName);
                            return false;
                        }
                        nodeId = dataRelationContract.Insert(dataRelationInfo);
                        name = dataRelationInfo.RelationName;
                        value = dataRelationInfo.RelationCode;
                    }
                    break;
            }
        ...
        private bool EditTreeNode(CommonNode commonNode, CombinedNodeType combinedNodeType, ref string verifyResult)
        {
            ...
            switch (combinedNodeType)
            {
                ...
                case CombinedNodeType.Leaf:
                    ...
                    {
                        DataRelationInfo dataRelationInfo = dataRelationModule.GetModelInfo();
                        DataRelationInfo oldDataRelationInfo = dataRelationContract.GetModelInfo(commonNode.NodeId);
                        dataRelationInfo.GroupId = oldDataRelationInfo.GroupId;
                        if (!dataRelationInfo.RelationName.Equals(oldDataRelationInfo.RelationName)
                            && dataRelationContract.IsExistIdenticalName(commonNode.NodeId, dataRelationInfo.RelationName))
                        {
                            Cursor = Cursors.Default;
                            verifyResult = string.Format("同一分类下的数据交换名称不允许重复, 数据交换名称[{0}]已存在。", oldDataRelationInfo.RelationName);
                            return false;
                        }
                        dataRelationContract.Update(dataRelationInfo);
                        name = dataRelationInfo.RelationName;
                    }
                    break;
            }
        ...
        protected override void InitFirstLevelNodes()
        {
            ...
            commonNodes.Add(new CommonNode(decimal.MinValue, decimal.MinValue, "数据转表结构", string.Empty, false, (byte)GroupType.DataRelation));
