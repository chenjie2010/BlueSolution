...
namespace AppFramework.Core
{
    ...
    public class QueryClauseConstructor
    {
        ...
        private IDictionary<string, string> _dataFieldNameRelations;
        ...
        /// 字段的物理名称与逻辑名称的对应关系
        /// </summary>
        public IDictionary<string, string> DataFieldNameRelations
        {
            get
            {
        ...
        public string BuildSqlSelectStatement(string whereClause)
        {
            ...
            {
                if (qf.Sorting != CustomSorting.None)
                {
                    sort = true;
                    break;
                }
        ...
        public string GetNewOrderBy()
        {
            ...
            foreach (QueryField qf in queryBuilder.QueryFields)
            {
                if (qf.Sorting != CustomSorting.None)
                {
                    ...
                    // descending
                    if (qf.Sorting == CustomSorting.Descending)
                        sb.Append(" DESC");
                }
            }
        ...
        private string BuildFromClause()
        {
            ...
            foreach (QueryField qf in queryBuilder.QueryFields)
            {
                ...
                {
                    DataFieldHelper.GetSystemTablePhysicalName((SystemDataField)Convert.ToByte(qf.DataFieldId));
                }
                if (qf.Output)
                {
                    if (sb.Length > 0)
        ...
        private string BuildWhereClause()
        {
            ...
            {                
                if (qf.Criteria.Length > 0)
                {
                    ...
                    sb.Append(")");
                    queryRealtion = qf.QueryDataFieldRealtion;
                }
            }
            return sb.ToString();
        }
        ...
        private string BuildGroupByClause()
        {
            ...
            foreach (QueryField qf in queryBuilder.QueryFields)
            {
                if (qf.CustomAggregate == Aggregate.GroupBy && qf.Output)
                {
                    // add separator
                    if (sb.Length > 0)
                        sb.Append(",\r\n\t");
                    ...
                    {
                        item = string.Format("{0}.{1}", qf.DataTableName, qf.ColumnName);
                    }
                    else
                    {
                        item = qf.ColumnName;
